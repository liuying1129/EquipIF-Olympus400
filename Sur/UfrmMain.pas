...
begin
  //if (MessageDlg('退出后将不能收到仪器数据,是否要退出？', mtConfirmation, [mbYes, mbNo], 0) = mrYes) then action:=cafree else action:=caNone;
...
begin
  ...
  //if (MessageDlg('退出后将不能收到仪器数据,是否要退出？', mtConfirmation, [mbYes, mbNo], 0) <> mrYes) then exit;
...
begin
  ...
  YXJB:=ini.ReadString(IniSection,'优先级别','自动');//优先级别
  ifP29:=ini.readBool(IniSection,'联机标识P改5',false);//联机标识P改9
...
begin
    result:=trim(COPY(TrimLeft(Value),10,4));//传过来的结果有可能#$2#$2'D...',故要TRIM
    result:='0000'+result;
...
begin
  ...
  begin
    ss:='串口选择'+#2+'Combobox'+#2+'COM1'+#13+'COM2'+#13+'COM3'+#13+'COM4'+#2+'0'+#2+#2+#3+
      ...
      '项目号开始位置'+#2+'Edit'+#2+#2+'1'+#2+'注:默认值46'+#2+#3+
      //'默认样本类型'+#2+'Edit'+#2+#2+'1'+#2+#2+#3+//仪器传过来
...
begin
  ...
  if length(TrimLeft(Str))<=11 then exit;//4->12
  ...
  if uppercase(copy(TrimLeft(str),1,1))='D' THEN//AU->HOST 检验结果
  BEGIN
    if YXJB='自动' then
      if uppercase(SpecNo[1])='P' then begin YXJB:='急诊';{SpecNo:=stringreplace(SpecNo,'P','0',[rfIgnoreCase]);} end else YXJB:='常规';
    if copy(TrimLeft(str),9,1)=' ' then SpecType:='血浆' else if copy(TrimLeft(str),9,1)='U' then SpecType:='尿液' else SpecType:='其他'; 

    if ifP29 then SpecNo:=StringReplace(SpecNo,'P','5',[rfReplaceAll, rfIgnoreCase]);
